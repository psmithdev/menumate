// GPT-4o Vision Smart Menu Parser (BACKUP)
// Preserved for testing and fallback scenarios
// Use smartMenuParser.ts for production (Google Cloud Vision)
...
/**
 * Parse menu using GPT-4 Vision API (BACKUP - SLOW & EXPENSIVE)
 * Use smartMenuParser.ts for production
 */
export async function parseMenuWithGPT4(
  imageFile: File
): Promise<SmartMenuResult> {
...
Return JSON:
{
  ...
  "totalDishes": count
    ...
    console.log(
      `✅ GPT-4o Validation: ${validation.filteredDishes.length} valid dishes, ${validation.rejectedCount} rejected`
    );
    ...
    console.error("GPT-4o menu parsing failed:", error);
    ...
    throw new Error("Failed to parse menu with GPT-4o");
  }
}
...
async function fileToBase64(file: File): Promise<string> {
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      console.log(
        `📷 Image optimized: ${file.size} bytes → ~${Math.round(
          base64.length * 0.75
        )} bytes`
      );
      resolve(base64);
